...
# CREATE SURFACES
s = SurfaceRZFourier.from_wout(wout, range="full torus", ntheta=ntheta, nphi=nphi)
cws = SurfaceRZFourier.from_nphi_ntheta(nphi, ntheta, "full torus", s.nfp)

R = s.get_rc(0, 0)
cws.set_dofs([R, s.get_zs(1, 0)*minor_radius_factor_cws, s.get_zs(1, 0)*minor_radius_factor_cws])

phi_array = np.linspace(0.01, 2*np.pi, ncoils, endpoint=False)
